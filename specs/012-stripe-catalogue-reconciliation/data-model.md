...
Catalogue reconciliation compares a Stripe product/price snapshot against canonical product mappings and intended retail pricing, emitting typed exceptions and approval-ready proposed fixes. Persistence uses Azure Blob (snapshots) + Azure Table (index) via Aspire-injected clients — **no SQL**.
...
Constructor: `(BlobServiceClient, TableServiceClient, IOptions<CatalogueReconciliationStorageOptions>)` — clients injected by Aspire; **no manual connection strings**.
