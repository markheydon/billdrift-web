...
**Storage access**: API orchestrates `ICatalogueReconciliationService` which persists via Aspire-injected blob/table clients in Infrastructure. Endpoints do not construct storage clients directly.
...
Either inline `productMappings` OR `mappingBlobPath` (future) required. Inline `stripeProducts` / `stripePrices` supported for v1 when ingestion blob loading not yet extended.
...
List catalogue reconciliation runs (paginated from table index).
...
Run detail with exception and proposed fix lists (loaded from blob archive).
...
`BillDrift.Web` consumes these endpoints via HTTP client — **no** direct `BlobServiceClient` / `TableServiceClient` in Web project. v1 validation uses API + quickstart scenarios.
