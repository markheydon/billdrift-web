...
**Feature**: `011-stripe-catalogue-reconciliation` → `007-reconciliation-approval-workflow`  
**Date**: 2026-07-03

## Purpose

Defines how catalogue proposed fixes enter the existing human approval queue without duplicating decision state.
...
| `CatalogueRunId` | Stored as `RunId` surrogate OR extension field `SourceCatalogueRunId` on proposal metadata |
